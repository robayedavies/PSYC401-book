...
2.  Specify what data we use `data = study.one.gen`
...
# study.one.gen
fit <- lm(mean.acc ~ SHIPLEY, data = study.one.gen)
# -- then
study.one.gen$predicted <- predict(fit)   # Save the predicted values
study.one.gen$residuals <- residuals(fit) # Save the residual values

# -- plot observed vs. predicted values
ggplot(study.one.gen, aes(x = SHIPLEY, y = mean.acc)) +
...
ggplot(study.one.gen, aes(x = residuals)) +
  geom_histogram(aes(y = ..density..), alpha = .75, binwidth = .1) +
  stat_function(fun = dnorm, args =
                  list(mean = mean(study.one.gen$residuals),
                       sd = sd(study.one.gen$residuals)),
                colour = "red", size = 1.5) +
